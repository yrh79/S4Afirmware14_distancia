...
void loop()
{
  ...
  if (millis()-timerCheckUpdate>=20)
  {
    ...
    reportLoadCell(); //loadCell value reporting
...
void reportLoadCell(){
  ...
  ScratchBoardSensorReport(1, v);   // LoadCell value
  ScratchBoardSensorReport(0, v);   // LoadCell value
}
...
//  for (sensorIndex = 0; sensorIndex < 5; sensorIndex++) // suprimimos la salida A5
  for (sensorIndex = 2; sensorIndex < 5; sensorIndex++) // A5 is used for ping, A0/A1 is used for loadCell
